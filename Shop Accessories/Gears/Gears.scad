...
/* Arrow bevel gear pair with arbitrary axis angle; uses the "pfeilkegelrad" module
    modul = Height of the tooth head above the pitch cone; specified for the outside of the cone
    zahnzahl_rad = Number of teeth on the wheel
    zahnzahl_ritzel = Number of teeth on the pinion
	achsenwinkel = Angle between the axes of wheel and pinion
    zahnbreite = Width of the teeth from the outside towards the cone apex
    bohrung_rad = Diameter of the central bore of the wheel
    bohrung_ritzel = Diameter of the central bore of the pinion
    eingriffswinkel = Pressure angle, default = 20° according to DIN 867. Should not be greater than 45°.
    schraegungswinkel = Helix angle, default = 0°
	zusammen_gebaut = Components assembled for construction or separated for 3D printing 
    */
    
module pfeilkegelradpaar(modul, zahnzahl_rad, zahnzahl_ritzel, axis_angle=90, zahnbreite, bohrung_rad, bohrung_ritzel, eingriffswinkel = 20, schraegungswinkel=10, zusammen_gebaut=true){
 
	r_rad = modul * zahnzahl_rad / 2;					// Pitch cone radius of the wheel
	delta_rad = atan(sin(axis_angle)/(zahnzahl_ritzel/zahnzahl_rad+cos(axis_angle)));	// Cone angle of the wheel
	delta_pinion = atan(sin(axis_angle)/(zahnzahl_rad/zahnzahl_ritzel+cos(axis_angle)));// Cone angle of the pinion
	rg = r_rad / sin(delta_rad);						// Radius of the large sphere
	c = modul / 6;									// Clearance
	df_pinion = pi * rg * delta_pinion / 90 - 2 * (modul + c); // Base cone diameter on the large sphere 
	rf_ritzel = df_pinion / 2;						// Base cone radius on the large sphere
	delta_f_pinion = rf_ritzel / (pi * rg) * 180;	// Head cone angle
	rkf_ritzel = rg * sin(delta_f_pinion);			// Radius of the cone base
	height_f_ritzel = rg * cos(delta_f_pinion);		// Height of the cone from the cone base
	
	echo("Cone Angle Wheel = ", delta_rad);
	echo("Cone Angle Pinion = ", delta_pinion);
 
	df_rad = pi * rg * delta_rad / 90 - 2 * (modul + c); // Base cone diameter on the large sphere 
	rf_rad = df_rad / 2;							// Base cone radius on the large sphere
	delta_f_rad = rf_rad / (pi * rg) * 180;		// Head cone angle
	rkf_rad = rg * sin(delta_f_rad);				// Radius of the cone base
	height_f_rad = rg * cos(delta_f_rad);			// Height of the cone from the cone base

	echo("Height Wheel = ", height_f_rad);
	echo("Height Pinion = ", height_f_ritzel);
	
	turn = is_even(zahnzahl_ritzel);
	
	// Wheel
	rotate([0,0,180*(1-spiel)/zahnzahl_rad*turn])
	...
	if (zusammen_gebaut)
		translate([-height_f_ritzel*cos(90-axis_angle),0,height_f_rad-height_f_ritzel*sin(90-axis_angle)])
			rotate([0,axis_angle,0])
				pfeilkegelrad(modul, zahnzahl_ritzel, delta_pinion, zahnbreite, bohrung_ritzel, eingriffswinkel, -schraegungswinkel);
	else
		translate([rkf_ritzel*2+modul+rkf_rad,0,0])
			pfeilkegelrad(modul, zahnzahl_ritzel, delta_pinion, zahnbreite, bohrung_ritzel, eingriffswinkel, -schraegungswinkel);
...
/*
Calculates a worm or Archimedean screw.
modul = Height of the worm head above the pitch cylinder
gangzahl = Number of threads (teeth) of the worm
laenge = Length of the worm
bohrung = Diameter of the central bore
eingriffswinkel = Pressure angle, default = 20° according to DIN 867. Should not be greater than 45°.
steigungswinkel = Helix angle of the worm, equivalent to 90° minus lead angle. Positive helix angle = right-hand.
zusammen_gebaut = Components assembled for construction or separated for 3D printing
*/

module worm(modul, gangzahl, laenge, bohrung, eingriffswinkel=20, steigungswinkel, zusammen_gebaut=true){

	// Dimension calculations
	c = modul / 6;  // Clearance
	r = modul * gangzahl / (2 * sin(steigungswinkel));  // Pitch cylinder radius
	rf = r - modul - c;  // Root cylinder radius
	a = modul * gangzahl / (90 * tan(eingriffswinkel));  // Spiral parameter
	tau_max = 180 / gangzahl * tan(eingriffswinkel);  // Angle from root to head in the normal direction
	gamma = -rad * laenge / ((rf + modul + c) * tan(steigungswinkel));  // Torsion angle for extrusion
	
	schritt = tau_max / 16;
	
	// Drawing: extrude a surface with twisting enclosed by two Archimedean spirals
	if (zusammen_gebaut) {
		rotate([0, 0, tau_max]){
							...
							polygon(
								concat(							
									...
									[for (tau = [0:schritt:tau_max])
										pol_to_cartesian([spiral(a, rf, tau), tau + i * (360 / gangzahl)])],
										
									// Tooth head
									[for (tau = [tau_max:schritt:180 / gangzahl])
										pol_to_cartesian([spiral(a, rf, tau_max), tau + i * (360 / gangzahl)])],
									
									// Falling tooth flank
									[for (tau = [180 / gangzahl:schritt:(180 / gangzahl+tau_max)])
										pol_to_cartesian([spiral(a, rf, 180 / gangzahl + tau_max - tau), tau + i * (360 / gangzahl)])]
								)
							);
						}
						circle(rf);
					}
					circle(bohrung / 2);  // Central bore
				}
			}
		}
	}
		...
		difference(){
			union(){
				translate([1, r * 1.5, 0]){
					rotate([90, 0, 90])
						worm(modul, gangzahl, laenge, bohrung, eingriffswinkel, steigungswinkel, zusammen_gebaut=true);
				}
...
/*
Calculates a worm gear set. The worm gear is a regular spur gear without globoid geometry.
modul = Height of the worm head above the pitch cylinder or the tooth head above the pitch circle
zahnzahl = Number of teeth on the wheel
gangzahl = Number of threads (teeth) of the worm
breite = Tooth width
laenge = Length of the worm
bohrung_schnecke = Diameter of the central bore of the worm
bohrung_rad = Diameter of the central bore of the spur gear
eingriffswinkel = Pressure angle, default = 20° according to DIN 867. Should not be greater than 45°.
steigungswinkel = Helix angle of the worm, equivalent to 90° - lead angle. Positive helix angle = right-hand.
optimiert = Holes for material/weight reduction
zusammen_gebaut = Components assembled for construction or separated for 3D printing
*/
...
module schneckenradsatz(modul, zahnzahl, gangzahl, breite, laenge, bohrung_schnecke, bohrung_rad, nabendurchmesser, nabendicke, eingriffswinkel=20, steigungswinkel, optimiert=true, zusammen_gebaut=true){
    ...
    c = modul / 6;                                              // Clearance
	r_schnecke = modul * gangzahl / (2 * sin(steigungswinkel)); // Worm pitch cylinder radius
	r_rad = modul * zahnzahl / 2;                               // Spur gear pitch cone radius
	rf_schnecke = r_schnecke - modul - c;                       // Root cylinder radius of the worm
	gamma = -90 * breite * sin(steigungswinkel) / (pi * r_rad); // Rotation angle of the spur gear
	zahnabstand = modul * pi / cos(steigungswinkel);            // Tooth spacing in the transverse section
